...
{	
	...
	if(!isset($_FILES['mFile']))
	{
            ...
            $query = mysql_query("UPDATE safari SET title= '$FileTitle',price='{$_POST['price']}',destination='{$_POST['dest']}',discr = '{$_POST['descr']}'  WHERE id='{$_POST['id']}'")or die(mysql_error());
            die("success");
	}else{
...
//        $layer->applyFilter(IMG_FILTER_BRIGHTNESS, 9, null, null, null, true);
        ...
        unlink($dirPath ."/".$_POST['imag']);
        //connect & insert file record in database
        $query = mysql_query("UPDATE safari SET title= '$FileTitle',price='{$_POST['price']}',destination='{$_POST['dest']}',discr = '{$_POST['descr']}',image='{$filename}' WHERE id='{$_POST['id']}'")or die(mysql_error());
        die("success");
   }else{
