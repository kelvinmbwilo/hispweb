...
{	
	...
	if(!isset($_FILES['mFile']))
	{
            ...
            $today = date("Y-m-d");
            $query = mysql_query("INSERT INTO resource VALUES ('', '$FileTitle','','{$today}','{$_POST['type']}','{$_POST['url']}')")or die(mysql_error());
            die("success");
	}else{
...
//   Rename and save uploded file to destination folder.
   ...
   {
        $today = date("Y-m-d");
        $query = mysql_query("INSERT INTO resource VALUES ('', '$FileTitle','{$NewFileName}','{$today}','{$_POST['type']}','{$_POST['url']}')")or die(mysql_error());
        die("success");
   }else{
